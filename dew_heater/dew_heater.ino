...
void setup() {
  ...
  if((value=='D')||(value=='A')){          //valid eeprom values found
    offsetmode=value;
...
void loop() {
  ...
  if((teletemp==0)|(teletemp>950)){thermerror=thermerror|1;}   //thermistor error detected
  teletemp=teletemp-238;
  if(teletemp<0){teletemp=0;}   //lookup table won't work if index<0
  ...
  if(key==btnLEFT){offsetmode='D';}      //offset from dewpoint
  if(key==btnRIGHT){offsetmode='A';}      //offset from ambient
  if(key==btnUP){tempoffset++;if(tempoffset>9){tempoffset=9;}}
  if(key==btnDOWN){tempoffset--;if(tempoffset<0){tempoffset=0;}}
  if(key==btnSELECT){savetoeeprom();}                    //save to eeprom (load happens on boot)
  ...
  if(thermerror){pwmoutput=pwmoutputdefault;}
  analogWrite(MOSFETPIN,pwmoutput);

  lcd.setCursor(0,0);
  switch(thermerror){
    case 0: LCDtopline();break;
    case 1: lcd.print("Thermistor fault");break;
    case 2: lcd.print("DHT11 fault     ");break;
    case 3: lcd.print("Thrm & DHT11 flt");break;
    default: lcd.print("Thrm & DHT11 flt");break;
  }
    
  lcd.setCursor(0,1);
  lcd.print("T:");
  if(teletemp>99){lcd.write((teletemp/100)%10+'0');}else{lcd.write(' ');}
  lcd.write((teletemp/10)%10+'0');
  lcd.print("C H:");
  pwmoutput=(pwmoutput*39)/100;      //scale to %
  if(pwmoutput>9){lcd.write((pwmoutput/10)%10+'0');}else{lcd.write(' ');}
...
void LCDtopline(){
  lcd.print("A:");
  if(ambtemp>99){lcd.write((ambtemp/100)%10+'0');}else{lcd.write(' ');}
  lcd.write((ambtemp/10)%10+'0');
  lcd.print("C ");
  if(ambhum>99){lcd.write((ambhum/100)%10+'0');}else{lcd.write(' ');}
  lcd.write((ambhum/10)%10+'0');
  lcd.print("% DP ");
  if(dptemp>99){lcd.write((dptemp/100)%10+'0');}else{lcd.write(' ');}
...
void savetoeeprom(){
  ...
  while(read_LCD_buttons()== btnSELECT){    //cycle while select held down 
    htr=htr+5;
    if(htr>99){htr=0;}
    lcd.setCursor(14,1);
    if(htr>9){lcd.write((htr/10)%10+'0');}else{lcd.write(' ');}
    lcd.write((htr)%10+'0');
    delay(500);
  }
  lcd.setCursor(0,0);
